...
* **Proportion of Uses Disclosed to Patients​**
  * Intended Use: Proportion of summaries for which use of the LLM is disclosed to the patient in a deployed system.
...
* **Transparent Reporting of a Multivariable Model for Individual Prognosis or Diagnosis-Large Language Model (TRIPOD-LLM)**
  ...
  * Reference: [The TRIPOD-LLM Statement: A Targeted Guideline For Reporting Large Language Models Use](https://pmc.ncbi.nlm.nih.gov/articles/PMC11361247/)
  * Open-source tooling:

* **Clinical-Grade Evaluation of Large Language Models​**
  * Intended Use: Evaluate presence/absence of reported elements in an evaluation. Checklist primarily intended for reproducibility, but has elements that can be surfaced for transparency, e.g., Model versionin
  * Rationale: Acknowledge but do not use.
  * Reference: [Toward Clinical-Grade Evaluation of Large Language Models](https://pmc.ncbi.nlm.nih.gov/articles/PMC11221761/)
  * Open-source tooling:
 ...
 * **Model Uptime/Failed Generations when Deployed**
   * Intended Use: Use existing red teaming/attack toolkits to evaluate for privacy leaks.
   * Rationale:
   * Reference: []()
   * Open-source tooling:
  
 * **Data Retention and Reuse Policie​s**
   * Intended Use: Document the threat model used in security/privacy analysis and provide clear justifications for which attacks are in scope vs. out of scope​.
   * Rationale:
   * Reference: []()
   * Open-source tooling:

 * **Minimum Data Access​**
   * Intended Use: If commercial, how much data a vendor/model has access to, compared to how much data is needed for task.
   * Rationale:
   * Reference: []()
   * Open-source tooling:
